...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'position_ids', 'old_log_probs', 'advantages',
                       'causal_mask', 'ref_log_prob', 'role_weights']
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for data in micro_batches:
                ...
                if entropy_top_ratio is not None:
                    entropy_top_mask = get_per_sample_entropy_top_mask(entropy=entropy, response_mask=response_mask,
                                                                       top_ratio=entropy_top_ratio)
                causal_mask = data['causal_mask']
                ref_logps = data['ref_log_prob']
                role_weights = data['role_weights']

                causal_merged = torch.any(causal_mask.bool(), dim=1)
                top_mask = torch.logical_or(
                    entropy_top_mask.bool(),
                    causal_merged
                ).long()
