...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_causal_mask(self, data: DataProto, causal_ratio=0.1):
        ...
        N_CF = 2
        all_cf_prompts_raw = []
        valid_cf_flags = []  # 长度 B*3，True=有效，False=None

        for info in extra_info_list:
            cf_list = info['cf_prompts'][:2]
            for pt in cf_list:
                if pt is not None:
                    all_cf_prompts_raw.append(pt)
                    valid_cf_flags.append(True)
                else:
                    all_cf_prompts_raw.append("")
                    valid_cf_flags.append(False)
        ...
        encoded = self.tokenizer(
            ...
            padding='max_length',
            return_tensors='pt'
        ).input_ids.to(device)  # (B*3, 2048), right-padded

        pad_token_id = self.tokenizer.pad_token_id
        batch_size_cf, max_len = encoded.shape
        actual_lengths = (encoded != pad_token_id).sum(dim=1)  # (B*3,)

        positions = torch.arange(max_len, device=encoded.device).unsqueeze(0).repeat(batch_size_cf, 1)
        valid_positions = positions >= (max_len - actual_lengths).unsqueeze(1)

        left_padded = torch.full_like(encoded, pad_token_id)

        if actual_lengths.sum() > 0:
            left_padded[valid_positions] = encoded[encoded != pad_token_id]

        cf_prompts_tensor = left_padded.to(device)  # (B*3, 2048)

        N_total = B * N_CF

        # ---  Repeat responses and masks ---
        resp_repeated = responses.unsqueeze(1).expand(-1, N_CF, -1).reshape(N_total, -1).to(device)  # (B*3, 2048)
        resp_mask_repeated = response_mask.unsqueeze(1).expand(-1, N_CF, -1).reshape(N_total, -1).to(
            device)  # (B*3, 2048)

        # --- Concatenate prompt + response ---
        cf_input_ids = torch.cat([cf_prompts_tensor, resp_repeated], dim=1)  # (B*3, 4096)
        prompt_mask = (cf_prompts_tensor != pad_token_id).long()  # (B*3, 2048)
        cf_attention_mask = torch.cat([prompt_mask, resp_mask_repeated], dim=1)  # (B*3, 4096)

        arange = torch.arange(cf_attention_mask.size(1), device=cf_attention_mask.device)
        cf_position_ids = (arange - cf_attention_mask.argmax(dim=1, keepdim=True)).clamp(min=0)
        ...
        # --- Reshape & Calculate Masks (核心修改部分) ---
        cf_log_probs = cf_data.batch['cf_log_probs'].view(B, N_CF, -1)  # (B, 3, 2048)

        # 计算差值：(B, 3, 2048) - (B, 1, 2048) -> (B, 3, 2048)
        abs_diff = (cf_log_probs - old_log_probs.unsqueeze(1)).abs()
        # response_mask: (B, 2048) -> (B, 1, 2048)
        abs_diff_masked = abs_diff.masked_fill(~response_mask.unsqueeze(1).bool(), -float('inf'))
        sorted_values, sorted_indices = torch.sort(abs_diff_masked, dim=2, descending=True)  # (B, 3, 2048)
        valid_counts = response_mask.sum(dim=1)  # (B,)
        k_per_sample = torch.clamp((valid_counts.float() * causal_ratio).long(), min=1).to(device)  # (B,)

        # Top-K Mask
        causal_mask = torch.zeros((B, N_CF, response_length), dtype=torch.long, device=device)
        for i in range(N_CF):
            indices_i = sorted_indices[:, i, :].to(device)
            arange_seq = torch.arange(response_length, device=device).unsqueeze(0)
            topk_flags = arange_seq < k_per_sample.unsqueeze(1)  # (B, 2048)
            src = topk_flags.to(torch.long).to(device)
            causal_mask[:, i, :].scatter_(dim=1, index=indices_i, src=src)

        valid_tensor = torch.tensor(valid_cf_flags, dtype=torch.long, device=device).view(B, N_CF)
        causal_mask = causal_mask * valid_tensor.unsqueeze(2)

        # 计算权重
        max_k = int(k_per_sample.max().item())
        max_k = min(max_k, response_length)
        vals_top = sorted_values[:, :, :max_k].to(device)
        idxs_top = sorted_indices[:, :, :max_k].to(device)  # [B, N_CF, max_k]

        pos_range = torch.arange(max_k, device=device).view(1, 1, -1)
        k_mask = (pos_range < k_per_sample.view(B, 1, 1))  # [B, 1, max_k] -> broadcast 到 [B, N_CF, max_k]
        log_vals = torch.log1p(torch.clamp(vals_top, min=0.0))
        log_vals_for_min = torch.where(
            k_mask,
            log_vals,
            torch.full_like(log_vals, float('inf'))
        ).to(device)
        log_vals_for_max = torch.where(
            k_mask,
            log_vals,
            torch.full_like(log_vals, float('-inf'))
        ).to(device)
        mn = log_vals_for_min.amin(dim=-1, keepdim=True)  # [B, N_CF, 1]
        mx = log_vals_for_max.amax(dim=-1, keepdim=True)  # [B, N_CF, 1]
        denom = (mx - mn).clamp(min=1e-9)

        betas_top = (log_vals - mn) / denom
        betas_top = torch.where(k_mask, betas_top, torch.zeros_like(betas_top))

        role_weights = torch.zeros((B, N_CF, response_length), dtype=torch.float, device=device)
        role_weights.scatter_(dim=2, index=idxs_top, src=betas_top)
        role_weights = role_weights * valid_tensor.unsqueeze(2)

        data.batch['causal_mask'] = causal_mask
        data.batch['role_weights'] = role_weights
